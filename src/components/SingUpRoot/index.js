...
class SignUpFormBaseRoot extends Component {
  constructor(props) {
    super(props);
    this.state = {
      username: '',
    ...
    return (
      <form onSubmit={this.onSubmit} className="">
        ...
        {/*<div className="select mb-2">
          <select className="select-text" name="parent" id="parent" onChange={this.onChange} defaultValue={"none"}>
            <option value="none" disabled>Parent</option>
            {this.state.parents.map((user,i) => <option key={i} value={user.uid}>{user.username}</option>)}
          </select>
        </div>*/}
        <div className="select mb-2">
...
const SignUpFormRoot = withRouter(withFirebase(SignUpFormBaseRoot));



export default
  withFirebase(SignUpPageRoot);

export { SignUpFormRoot };
